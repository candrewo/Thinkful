...
def no_more_nests(array, array2):
    array_for_testing = []
    for number1, number2 in zip(array, array2):
    	big_number = calculate_big_number(number1, number2)
    	new_number = number2 * number1
        if number1 > number2:
            big_number 	#big_number is calculated if number 1 is greater than 2
        if number2 - big_number < number1:
            number2 = min(number2, big_number) #number 2 is changed to the smaller value if the condition set in the if statement is met
        if number1 ** 2 - number2 ** 2 > 0:
             continue
...
print no_more_nests(evens_odds, odds_evens) 
